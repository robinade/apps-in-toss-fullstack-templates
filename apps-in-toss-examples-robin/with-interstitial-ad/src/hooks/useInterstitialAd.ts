...
function enqueue(fn: () => void) {
  if (!_loading) { _loading = true; fn(); } else { _queue.push(fn); }
}
function dequeue() {
  _loading = false;
...
export function useInterstitialAd(adGroupId?: string) {
  ...
  const callbacksRef = useRef<InterstitialAdCallbacks>({});
  ...
  const showAd = useCallback(async (callbacks: InterstitialAdCallbacks): Promise<boolean> => {
    if (loading || !isReady) {
    ...
    return true;
  }, [loading, isReady, checkSupport, groupId, addEvent, loadAd]);

  return { loading, isReady, retryCount, adViewCount, loadAd: () => loadAd(0), showAd };
}
